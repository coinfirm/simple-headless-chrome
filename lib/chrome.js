...
module.exports.launch = async function (headless, options = {}) {
  ...
  if (headless && chromeOptions.chromeFlags.indexOf('--headless') === -1) { chromeOptions.chromeFlags.push('--headless') }
  if (options.disableGPU && chromeOptions.chromeFlags.indexOf('--disable-gpu') === -1) { chromeOptions.chromeFlags.push('--disable-gpu') }
  if (options.noSandbox && chromeOptions.chromeFlags.indexOf('--no-sandbox') === -1) { chromeOptions.chromeFlags.push('--no-sandbox') }
...
module.exports.attachCdpToBrowser = async function (host = 'localhost', port = 9222, remote = false) {
  ...
  try {
    let client = await CDP({ host, port, remote })
