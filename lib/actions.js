...
exports.printToPDF = async function (options = {}, returnBinary = false) {
  ...
  console.log(`:: printToPDF => Printing page to PDF with options: ${JSON.stringify(options, null, 2)}. Returning as ${returnBinary ? 'binary' : 'base64'} string`)
  browserIsInitialized.call(this)
  console.log(':: printToPDF => browserInitilized')
  try {
    let pdf = (await this.client.Page.printToPDF(options)).data
    console.log(':: printToPDF => Converting to base64')
    if (returnBinary) {
    ...
    console.log(`:: printToPDF => ERR - ${err}`)
  }
}
...
exports.savePdf = async function (fileName = `pdf-${Date.now()}`, options = {}) {
  ...
  console.log(':: savePdf => Saving a PDF of the page...');
  browserIsInitialized.call(this)
  console.log(':: savePdf => browser initialized, running writeFile...');
  await fs.writeFile(`${fileName}.pdf`, await this.printToPDF(options, true))
  console.log(':: savePdf => PDF saved!');
